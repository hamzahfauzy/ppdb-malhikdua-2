...
class HomeController extends Controller
{
    ...
    public function formulir(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if($contact->formulir)
            {
                $contact->formulir->status = $request->status;
                $contact->formulir->save();
                // file validation
            ...
            else
            {
                ...
                {
                    $request->validate([
                        'upload_kk' => 'required|mimes:jpeg,bmp,png,gif,svg,pdf|max:5024',
                        'upload_akte' => 'required|mimes:jpeg,bmp,png,gif,svg,pdf|max:5024',
                        'upload_izajah' => 'nullable|mimes:jpeg,bmp,png,gif,svg,pdf|max:5024',
                        'upload_shun' => 'nullable|mimes:jpeg,bmp,png,gif,svg,pdf|max:5024',
                        'upload_kartu_pemerintah' => 'nullable|max:5024',
                        'upload_ktp_ayah' => 'nullable|max:5024',
                        'upload_ktp_ibu' => 'nullable|max:5024',
                        'upload_kip' => 'nullable|max:5024',
                        'upload_kks' => 'nullable|max:5024',
                        'upload_pkh' => 'nullable|max:5024',
                    ]);
                }
                DB::beginTransaction();
                try {
                    ...
                    if ($nf = $formulir->create(['contact_id' => $contact->id, 'status' => $request->status])) {
                        $tiket = $nf->id;
                        if($tiket < 10)
                            $tiket = "000".$tiket;
                        elseif($tiket < 100)
    ...
    function send()
    {
        ...
        $wa = new WaBlast;
        // send to pendaftar
        $message = "Data isian kakak *".$formulir->diri->nama_lengkap."* telah kami terima untuk diverifikasi lebih lanjut oleh petugas. Hubungi Petugas PPDB jika sampai 3 hari belum ada kabar terkait hasil verifikasi";
        ...
        $message = "Pendaftar baru a.n : *".$formulir->diri->nama_lengkap."* asal *".$formulir->asal->kabupaten."*, pilihan program/spesifikasi *".$formulir->rencana->program."* telah mengirimkan form/berkas pendaftaran. Segera lakukan *VERIFIKASI* melalui tombol tersedia di panel operator.";
        $operator = Staff::where('is_admin', 0)->get();
        ...
        try {
            ...
            $spreadsheetId = env('SPREADSHEET_ID','1xUz-01WZe6mand08L8kP9wmWYWr6qsgbrRAG0YhL0Bw');
            $range = 'Sheet1!A:X'; 
            ...
            $values = [
                [
                    ...
                    $formulir->diri->tempat_tinggal,
                    $formulir->diri->tanggal_lahir,
                    $formulir->diri->jenis_kelamin,
                    $formulir->diri->no_kk,
                    $formulir->diri->NIK,
                    $formulir->diri->anak_ke,
                    $formulir->diri->jumlah_saudara,
                    $formulir->diri->agama,
                    $formulir->diri->cita_cita,
                    $formulir->diri->email,
                    $formulir->diri->yang_membiayai_sekolah,
                    $formulir->diri->nama_kepala_keluarga,
                    $formulir->pendidikan->NISN,
                    $formulir->pendidikan->sekolah_asal,
                    $formulir->pendidikan->NPSN,
                    $formulir->pendidikan->angkatan_lulus,
                    $formulir->pendidikan->alamat,
                    $formulir->pendidikan->NSM,
                    $formulir->pendidikan->prestasi,
                    $formulir->data_rencana_sekolah->program,
                    $formulir->data_rencana_sekolah->spesifikasi,
                    $formulir->data_rencana_sekolah->extra_peminatan,
                ]
            ];
    ...
    public function getStatusPpdb(Request $request)
    {
        \Log::info($request);
        $phone = substr($request->from, 2);
        $user  = User::where('email','LIKE', '%'.$phone.'%')->with(['contact.formulir'])->first();

        if($user)
        {
            $status = 'TIDAK DIKETAHUI';
            if(!$user->contact->tiket)
            {
                $status = 'MENUNGGU PEMBAYARAN';
            }
            if($user->contact->tiket)
            {
                $status = 'SUSKES PEMBAYARAN DAN MENUNGGU PENGIRIMAN DATA FORMULIR';
            }
            if($user->contact->formulir)
            {
                if($user->contact->formulir->status == 'Dikirim')
                {
                    $status = 'BERKAS/DATA FORMULIR DALAM VERIFIKASI PETUGAS';
                }
                if($user->contact->formulir->status == 'Diterima')
                {
                    $status = 'BERKAS LOLOS VERIFIKASI DAN BERSIAP TEST SELEKSI';
                }
                if($user->contact->formulir->status == 'Lulus')
                {
                    $status = 'PENDAFTAR DITERIMA MALHIKDUA DAN MENUNGGU DAFTAR ULANG';
                }
            }

            $message = 'Status/tahap pendaftaran kakak saat ini :
*'. $status .'*';
        }
        else
        {
            $message = 'Nomer WA kakak tidak terdaftar dalam system PPDB Malhikdua.';
        }
