...
class SiswaController extends Controller
{
    ...
    public function index()
    {
        ...
        $siswa = Formulir::whereNotIn('status',['','final','draft'])->get();
    ...
    function approve(Formulir $formulir)
    {
        ...
        $program = [
            'Keagamaan' => 'Keagamaan',
            'Olimpiade (IPA Unggulan)' => 'IPA',
            'Vokasi (IPA Regular)' => 'IPA',
            'Olimpiade (IPS Unggulan)' => 'IPS',
            'Vokasi (IPS Regular)' => 'IPS'
        ];

        $wag = $wags[$formulir->rencana->program];
        $program = $program[$formulir->rencana->program];

        $message = "Selamat, data dan berkas kakak ".$formulir->diri->nama_lengkap." telah lolos verifikasi. Untuk tahap selanjutnya dimohon bergabung dengan *WAG ".$program."* dengan klik link undangan WAG berikut ". $wag;
        $wa->send_text("62".$formulir->contact->no_wa,$message);

        $jumlahPendaftar = Formulir::whereNotIn('status',['','final','draft'])->count();
        $jumlahPendaftar = number_format($jumlahPendaftar, 0, ',', '.');
        
        $keagamaan = Formulir::whereNotIn('status',['','final','draft'])->whereHas('data_rencana_sekolah', function($query){
            $query->where('program', 'Keagamaan');
        })->count();
        $keagamaan = number_format($keagamaan, 0, ',', '.');

        $olimpiadeIpa = Formulir::whereNotIn('status',['','final','draft'])->whereHas('data_rencana_sekolah', function($query){
            $query->where('program', 'Olimpiade (IPA Unggulan)');
        })->count();
        $olimpiadeIpa = number_format($olimpiadeIpa, 0, ',', '.');
        
        $olimpiadeIps = Formulir::whereNotIn('status',['','final','draft'])->whereHas('data_rencana_sekolah', function($query){
            $query->where('program', 'Olimpiade (IPS Unggulan)');
        })->count();
        $olimpiadeIps = number_format($olimpiadeIps, 0, ',', '.');
        
        $vokasiIpa = Formulir::whereNotIn('status',['','final','draft'])->whereHas('data_rencana_sekolah', function($query){
            $query->where('program', 'Vokasi (IPA Regular)');
        })->count();
        $vokasiIpa = number_format($vokasiIpa, 0, ',', '.');
        
        $vokasiIps = Formulir::whereNotIn('status',['','final','draft'])->whereHas('data_rencana_sekolah', function($query){
            $query->where('program', 'Vokasi (IPS Regular)');
        })->count();
        $vokasiIps = number_format($vokasiIps, 0, ',', '.');

        $operator = \App\Models\Staff::where('is_admin', 0)->get();
        $message = "Pendaftar baru a.n : *".$formulir->diri->nama_lengkap."* asal *".$formulir->asal->kabupaten."*, pilihan program/spesifikasi *".$formulir->rencana->program."* telah lolos verifikasi. Mohon pastikan nama *".$formulir->diri->nama_lengkap."* nomer WA *+62".$formulir->contact->no_wa."* telah masuk ke WAG sesuai program yang dipilih.

*Rekap PPDB per detik ini:*
Total jumlah Pendaftar : $jumlahPendaftar
Keagamaan : $keagamaan
Olimpiade (IPA Unggulan) : $olimpiadeIpa
Olimpiade (IPS Unggulan) : $olimpiadeIps
Vokasi (IPA Regular) : $vokasiIpa
Vokasi (IPS Regular) : $vokasiIps

Target pendaftar tahun ini berjumlah 500
Pendaftar tahun lalu berjumlah 409.";

        foreach($operator as $op)
        {
            $wa->send_text('62'.$op->phone_number,$message);
        }
    ...
    function lulus(Formulir $formulir)
    {
        ...
        $wa = new WaBlast;
        $message = "Assalamualaikum. Selamat atas keberhasilan kakak ".$formulir->diri->nama_lengkap." dalam seleksi PPDB Malhikdua School tahun ajaran 2025/2026. Kami mengundang kakak untuk bergabung dengan keluarga besar Malhikdua School. Sebagai informasi, pelaksanaan DAFTAR ULANG wajib dilakukan sesuai dengan ketentuan agar tidak dinyatakan mengundurkan diri. Mari bersama-sama kita ciptakan masa depan yang cerah bersama MALHIKDUA!";
        $wa->send_text("62".$formulir->contact->no_wa,$message);
        return redirect()->route('staff.siswa.show',$formulir->id)->with(['success'=>'Berhasil Terima Pendaftar']);
    }
