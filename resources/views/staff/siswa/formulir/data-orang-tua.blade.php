<h4>Data Ayah</h4>
<input type="hidden" name="ayah[no_kk]" value="0">
<div class="form-group">
    <label for="">Nama ayah</label>
    <input type="text" name="ayah[nama]" class="form-control">
</div>
<div class="form-group">
    <label for="">Status ayah</label>
    <select name="ayah[status]" class="form-control">
        <option value="-" selected disabled>- Pilih Status -</option>
        <option value="Kandung">Kandung</option>
        <option value="Angkat">Angkat</option>
        <option value="Tiri">Tiri</option>
    </select>
</div>
<div class="form-group">
    <label for="">NIK Ayah</label>
    <input type="tel" pattern="[0-9]{16}" name="ayah[no_kk_ayah]" class="form-control">
</div>
<div class="form-group">
    <label for="">Tempat lahir ayah</label>
    <input type="text" name="ayah[tempat_lahir]" class="form-control">
</div>
<div class="form-group">
    <label for="">Tanggal lahir ayah</label>
    <input type="date" name="ayah[tanggal_lahir]" class="form-control">
</div>
<div class="form-group">
    <label for="">Keadaan Ayah</label>
    <select name="ayah[keadaan]" class="form-control">
        <option value="-" selected disabled>- Pilih Keadaan Hidup -</option>
        <option value="Hidup">Hidup</option>
        <option value="Meninggal">Meninggal</option>
    </select>
</div>
<div class="form-group">
    <label for="">No Handphone ayah</label>
    <input type="text" name="ayah[no_handphone]" class="form-control">
</div>
<div class="form-group">
    <label for="">Pekerjaan Ayah</label>
    <select name="ayah[pekerjaan]" class="form-control">
        <option>tidak bekerja</option>
        <option>pensiunan</option>
        <option>PNS (selain poin 05 dan 10)</option>
        <option>TNI/Polisi</option>
        <option>Guru/dosen</option>
        <option>pegawai swasta</option>
        <option>wiraswasta/wirausaha</option>
        <option>pengacara/hakim/jaksa/notaris</option>
        <option>seniman/pelukis/artis/sejenisnya</option>
        <option>okter/bidan/perawat</option>
        <option>pilot/pramugara</option>
        <option>pedagang</option>
        <option>petani/peternak</option>
        <option>nelayan</option>
        <option>buruh (tani/pabrik/bangunan)</option>
        <option>sopir/masinis/kondektur</option>
        <option>politikus</option>
        <option>lainnya</option>
    </select>
</div>
<div class="form-group">
    <label for="">Pendidikan Ayah</label>
    <select name="ayah[pendidikan]" class="form-control">
        <option>Tidak berpendidikan</option>
        <option>SD/Sederajat</option>
        <option>SMP/Sederajat</option>
        <option>SMA/Sederajat</option>
        <option>D1</option>
        <option>D2</option>
        <option>D3</option>
        <option>D4/S1</option>
        <option>S2</option>
        <option>S3</option>
    </select>
</div>
<div class="form-group">
    <label for="">Penghasilan</label>
    <select name="ayah[penghasilan]" class="form-control">
        <option value="kurang dari Rp 500.000">kurang dari Rp 500.000</option>
        <option value="Rp 500.000 - 1.000.000">Rp 500.000 - 1.000.000</option>
        <option value="Rp 1.000.001 - 2.000.000">Rp 1.000.001 - 2.000.000</option>
        <option value="Rp 2.000.001 - 3.000.000">Rp 2.000.001 - 3.000.000</option>
        <option value="Rp 3.000.001 - 5.000.000">Rp 3.000.001 - 5.000.000</option>
        <option value="lebih dari 5.000.000">lebih dari 5.000.000</option>
    </select>
</div>

<h4>Data Ibu</h4>
<input type="hidden" name="ibu[no_kk]" value="0">
<div class="form-group">
    <label for="">Nama</label>
    <input type="text" name="ibu[nama]" class="form-control">
</div>
<div class="form-group">
    <label for="">Status</label>
    <select name="ibu[status]" class="form-control">
        <option value="-" selected disabled>- Pilih Status -</option>
        <option value="Kandung">Kandung</option>
        <option value="Angkat">Angkat</option>
        <option value="Tiri">Tiri</option>
    </select>
</div>
<div class="form-group">
    <label for="">NIK</label>
    <input type="tel" pattern="[0-9]{16}" name="ibu[no_kk_ibu]" class="form-control">
</div>
<div class="form-group">
    <label for="">Tempat lahir ibu</label>
    <input type="text" name="ibu[tempat_lahir]" class="form-control">
</div>
<div class="form-group">
    <label for="">Tanggal lahir</label>
    <input type="date" name="ibu[tanggal_lahir]" class="form-control">
</div>
<div class="form-group">
    <label for="">Keadaan</label>
    <select name="ibu[keadaan]" class="form-control">
        <option value="-" selected disabled>- Pilih Keadaan Hidup -</option>
        <option value="Hidup">Hidup</option>
        <option value="Meninggal">Meninggal</option>
    </select>
</div>
<div class="form-group">
    <label for="">No Handphone ibu</label>
    <input type="text" name="ibu[no_handphone]" class="form-control">
</div>
<div class="form-group">
    <label for="">Pekerjaan</label>
    <select name="ibu[pekerjaan]" class="form-control">
        <option>tidak bekerja</option>
        <option>pensiunan</option>
        <option>PNS (selain poin 05 dan 10)</option>
        <option>TNI/Polisi</option>
        <option>Guru/dosen</option>
        <option>pegawai swasta</option>
        <option>wiraswasta/wirausaha</option>
        <option>pengacara/hakim/jaksa/notaris</option>
        <option>seniman/pelukis/artis/sejenisnya</option>
        <option>okter/bidan/perawat</option>
        <option>pilot/pramugara</option>
        <option>pedagang</option>
        <option>petani/peternak</option>
        <option>nelayan</option>
        <option>buruh (tani/pabrik/bangunan)</option>
        <option>sopir/masinis/kondektur</option>
        <option>politikus</option>
        <option>lainnya</option>
    </select>
</div>
<div class="form-group">
    <label for="">Pendidikan</label>
    <select name="ibu[pendidikan]" class="form-control">
        <option>Tidak berpendidikan</option>
        <option>SD/Sederajat</option>
        <option>SMP/Sederajat</option>
        <option>SMA/Sederajat</option>
        <option>D1</option>
        <option>D2</option>
        <option>D3</option>
        <option>D4/S1</option>
        <option>S2</option>
        <option>S3</option>
    </select>
</div>
<div class="form-group">
    <label for="">Penghasilan</label>
    <select name="ibu[penghasilan]" class="form-control">
        <option value="kurang dari Rp 500.000">kurang dari Rp 500.000</option>
        <option value="Rp 500.000 - 1.000.000">Rp 500.000 - 1.000.000</option>
        <option value="Rp 1.000.001 - 2.000.000">Rp 1.000.001 - 2.000.000</option>
        <option value="Rp 2.000.001 - 3.000.000">Rp 2.000.001 - 3.000.000</option>
        <option value="Rp 3.000.001 - 5.000.000">Rp 3.000.001 - 5.000.000</option>
        <option value="lebih dari 5.000.000">lebih dari 5.000.000</option>
    </select>
</div>

<h4>Data Wali</h4>
<div class="form-group">
    <label for="">Wali</label>
    <select name="wali[opsi]" class="form-control">
        <option value="Sama dengan ayah kandung">Sama dengan ayah kandung</option>
        <option value="Sama dengan ibu kandung">Sama dengan ibu kandung</option>
        <option value="Lainnya">Lainnya</option>
    </select>
</div>
<div id="wali-box" class="d-none">
    <div class="form-group">
        <label for="">Nama wali</label>
        <input type="text" name="wali[nama]" class="form-control">
    </div>
    <div class="form-group">
        <label for="">NIK Wali</label>
        <input type="tel" pattern="[0-9]{16}" name="wali[NIK]" class="form-control">
    </div>
    <div class="form-group">
        <label for="">Tempat lahir</label>
        <input type="text" name="wali[tempat_lahir]" class="form-control">
    </div>
    <div class="form-group">
        <label for="">Tanggal lahir</label>
        <input type="date" name="wali[tanggal_lahir]" class="form-control">
    </div>
    <div class="form-group">
        <label for="">Hubungan dengan pendaftar</label>
        <input type="text" name="wali[hubungan_dengan_pendaftar]" class="form-control">
    </div>
    <div class="form-group">
        <label for="">No Handphone wali</label>
        <input type="text" name="wali[no_handphone]" class="form-control">
    </div>
    <div class="form-group">
        <label for="">Pekerjaan</label>
        <select name="wali[pekerjaan]" class="form-control">
            <option>tidak bekerja</option>
            <option>pensiunan</option>
            <option>PNS (selain poin 05 dan 10)</option>
            <option>TNI/Polisi</option>
            <option>Guru/dosen</option>
            <option>pegawai swasta</option>
            <option>wiraswasta/wirausaha</option>
            <option>pengacara/hakim/jaksa/notaris</option>
            <option>seniman/pelukis/artis/sejenisnya</option>
            <option>okter/bidan/perawat</option>
            <option>pilot/pramugara</option>
            <option>pedagang</option>
            <option>petani/peternak</option>
            <option>nelayan</option>
            <option>buruh (tani/pabrik/bangunan)</option>
            <option>sopir/masinis/kondektur</option>
            <option>politikus</option>
            <option>lainnya</option>
        </select>
    </div>
    <div class="form-group">
        <label for="">Pendidikan</label>
        <select name="wali[pendidikan]" class="form-control">
            <option>Tidak berpendidikan</option>
            <option>SD/Sederajat</option>
            <option>SMP/Sederajat</option>
            <option>SMA/Sederajat</option>
            <option>D1</option>
            <option>D2</option>
            <option>D3</option>
            <option>D4/S1</option>
            <option>S2</option>
            <option>S3</option>
        </select>
    </div>
    <div class="form-group">
        <label for="">Penghasilan</label>
        <select name="wali[penghasilan]" class="form-control">
            <option value="kurang dari Rp 500.000">kurang dari Rp 500.000</option>
            <option value="Rp 500.000 - 1.000.000">Rp 500.000 - 1.000.000</option>
            <option value="Rp 1.000.001 - 2.000.000">Rp 1.000.001 - 2.000.000</option>
            <option value="Rp 2.000.001 - 3.000.000">Rp 2.000.001 - 3.000.000</option>
            <option value="Rp 3.000.001 - 5.000.000">Rp 3.000.001 - 5.000.000</option>
            <option value="lebih dari 5.000.000">lebih dari 5.000.000</option>
        </select>
    </div>

</div>